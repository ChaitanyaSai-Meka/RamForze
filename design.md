...
The Worker also rate limits handshake attempts per source IP, using `conn.RemoteAddr()` from the TCP connection itself rather than any client-supplied field, since a self-reported IP could be spoofed by an attacker. Each IP is limited to 5 handshake attempts per fixed 1-minute window; requests beyond that are rejected until the window resets. This is a v1 fixed-window rate limiter, not a lockout - an IP that hits the limit is free to try again once the next window starts, and no longer-term ban is applied for repeated offenses. A fixed window has a known edge case (bursts near a window boundary can allow up to double the stated rate in a short span), which is accepted as a reasonable trade-off for a LAN-only tool; a sliding window or lockout escalation are noted as possible future hardening, not required for v1.
